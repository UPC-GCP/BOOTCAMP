...
void CG::solve(std::vector<Matrix> matA, std::vector<double>& x, std::vector<double> matB, std::vector<int> ignoreBC) {

    // Control
    int n = matB.size(); double alpha, rsNew, beta;
    std::vector<double> r = matB, Ap(n);

    // Residual
    std::vector<double> Ax = operProdMatVec(matA, x);
